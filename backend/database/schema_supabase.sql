-- MobiMEA Intelligence Platform - Supabase Schema
-- (Supabase-compatible version without TimescaleDB)
...
-- Prices table (standard PostgreSQL version)
...
CREATE INDEX IF NOT EXISTS idx_scraper_logs_retailer ON scraper_logs(retailer, created_at DESC);
...
('Emtel', 'https://www.emtel.com', NULL, 'traditional'),
('JKalachand', 'https://www.jkalachand.com', NULL, 'traditional')
...
SELECT DISTINCT ON (rl.product_id, rl.retailer_id)
    ...
    r.id as retailer_id,
    r.name as retailer,
    pr.price_cash as price,
    pr.price_credit,
    pr.original_price,
    pr.in_stock,
    pr.stock_status,
    pr.promo_text,
    rl.original_url as url,
...
SELECT
    ...
    (SELECT MAX(created_at) FROM scraper_logs) as last_scrape_time;
